...
class WebsiteSale(http.Controller):
    ...
    def checkout_form_validate(self, mode, all_form_values, data):
        ...
        required_fields += mode[
                               1] == 'shipping' and self._get_mandatory_fields_shipping(
            country_id) or self._get_mandatory_fields_billing(country_id)
        if mode[1] == 'shipping':
            if not request.env['ir.config_parameter'].sudo().get_param(
                    'website_sale_address_management.shipping_phone_is_required'):
                if 'phone' in required_fields:
                    required_fields.remove('phone')
            if not request.env['ir.config_parameter'].sudo().get_param(
                    'website_sale_address_management.shipping_zip_code_is_required'):
                if 'zip' in required_fields:
                    required_fields.remove('zip')
            if not request.env['ir.config_parameter'].sudo().get_param(
                    'website_sale_address_management.shipping_street_is_required'):
                if 'street' in required_fields:
                    required_fields.remove('street')
            if not request.env['ir.config_parameter'].sudo().get_param(
                    'website_sale_address_management.shipping_city_is_required'):
                if 'city' in required_fields:
                    required_fields.remove('city')
        elif mode[1] == 'billing':
            if not request.env['ir.config_parameter'].sudo().get_param(
                    'website_sale_address_management.billing_phone_is_required'):
                if 'phone' in required_fields:
                    required_fields.remove('phone')
            if not request.env['ir.config_parameter'].sudo().get_param(
                    'website_sale_address_management.billing_zip_code_is_required'):
                if 'zip' in required_fields:
                    required_fields.remove('zip')
            if not request.env['ir.config_parameter'].sudo().get_param(
                    'website_sale_address_management.billing_street_is_required'):
                if 'street' in required_fields:
                    required_fields.remove('street')
            if not request.env['ir.config_parameter'].sudo().get_param(
                    'website_sale_address_management.billing_city_is_required'):
                if 'city' in required_fields:
                    required_fields.remove('city')
