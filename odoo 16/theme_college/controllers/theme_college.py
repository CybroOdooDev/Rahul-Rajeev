...
class College(http.Controller):
    ...
    @http.route('/college_location/<int:college_location_id>', type='http', auth='public',
                website=True)
    def view_country_details(self, college_location_id):
        """"function to render template and pass value to website"""
        college_location = request.env['college.location'].sudo().search([('id', '=', college_location_id)])
        values = {
            'college_location': college_location
        }
